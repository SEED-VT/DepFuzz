...
object JazzerTargetCommuteTypeFull {
  ...
  def fuzzerTestOneInput(data: FuzzedDataProvider): Unit = {
    ...
    val f: Array[String] => Unit = pkg match {
      case "faulty" => examples.faulty.CommuteTypeFull.main
      case _ => throw new NotImplementedError() //examples.fuzzable.CommuteType.main
    }
